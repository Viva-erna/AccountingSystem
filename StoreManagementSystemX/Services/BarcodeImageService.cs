...
namespace StoreManagementSystemX.Services
{
    public class BarcodeImageService : IBarcodeImageService
    {
        ...
        {
            // ДОБАВЛЕНА ПРОВЕРКА
            if (string.IsNullOrEmpty(barcode) || !barcode.All(char.IsDigit) || barcode.Length != 12)
            {
                // Обработка ошибки: некорректный штрихкод
                // Возвращаем null или бросаем исключение, в зависимости от логики приложения
                return null; // Или throw new ArgumentException("Invalid barcode format.");
            }
